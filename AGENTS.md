...
| **Qualoop 升级到 L2 / L3** | 修改 `qualoop.json` 的 `maturity`，并按 `tools/qualoop/ADOPTION_GUIDE.md` Phase 2/3 启用 Scheduler / Executor 链路；继续受三前提约束。当升级到 L3 时，自动在本地终端后台拉起 `.\automation\start.bat` 来启动 Guardian 守护进程，从而实现整个监测与修复环在系统后台 24 小时常驻运行。 |
...
## 9. Qualoop 自身升级与维护规则 (Qualoop Self-Upgrade Rule)

- **升级记录强制要求**：任何对 Qualoop 框架代码（包括 `automation/` 目录下的 tester、scorer、planner、scheduler、guardian 等核心组件或配置模板）的修改或功能升级，**必须同步追加记录**到网页版报告 [development-report.html](file:///E:/20260502_MZH/Qualoop/reports/development-report.html) 中。
- **自动更新网页版报告**：任何对 Qualoop 框架代码的更新和推送，必须确保伴随运行 `qualoop.py check`，将升级记录自动持久化更新到网页版报告 `development-report.html` 后，方可一并 commit 并 push 到 GitHub。禁止直接在未生成报告的情况下提交代码。
- **自动化校验**：此规则已被编写进 Qualoop 测试器 `automation/tester.py` 的 `check_qualoop_self_upgrade` 检查项中。如果修改了框架 `.py` 脚本但未修改 `development-report.html`，Qualoop 检查将判定为不达标。
- **记录规范**：升级记录应包含修改日期、升级组件、严重程度、问题影响及解决状态，禁止任何在未更新报告情况下的直接发版或提交。

---

## 10. 目标导向调试与改进判定规则 (Goal-Oriented Debug & Improvement Rule)

- **目标导向审计**：在发现问题与提出改进意见时，必须以业务项目的最终目标（如 `GOALS.md`）为唯一判据。所有探测机制必须像人类调试者一样对真实系统运行态、数据库及接口进行深层校验，不能脱离最终目标提出无价值的表面代码调整。
- **防假绿/Mock假绿机制**：禁止仅依靠 HTTP 200 或接口返回成功作为通过条件。对系统生成的各种交付介质/文件格式（如音频、视频、PDF、PNG、JSON 结构体等），测试器必须验证其实质性的文件头魔术字节（如 `ID3`, `ftyp`, `%PDF`, `\x89PNG`）和数据合法性，严防损坏的空文件或占位符欺骗测试。
- **同步与双向校验**：为防止框架记录与真实代码脱节，对 Qualoop 的功能升级不仅要在 `development-report.html` 中同步记录，且必须通过 `check_qualoop_self_upgrade` 的自动化双向校验，保证升级记录中描述的修改与代码实现相匹配，并在每次框架发版或修改后及时将变更同步至业务应用项目的副本（如 `ParadigmLearn/automation/`）中。
