...
| **插件化与治理** | 硬编码在 scripts 目录，依赖特定接口 | **Semantic Kernel**: 标准化插件目录与依赖注入<br>**Pydantic AI**: 基于 Pydantic 强类型约束 of Tool 依赖注入<br>**Agency**: 基于 Actor 权限模型与 ACL 的工具治理<br>**TaskWeaver**: 代码首要 (Code-First) 规划与动态 Tool 绑定 | **高**：探针与动作插件化，结合 Pydantic AI 的依赖注入和 Agency 权限拦截器，参考 TaskWeaver 引入动态 Python 代码生成与沙盒执行以增强 Tool 扩展性。 |
