...
| **可观测与可审计** | 静态生成 markdown 报告与 json 状态 | **OpenHands**: Append-Only Event Stream 日志<br>**Langfuse**: 基于 OpenTelemetry 的分层 Traces 追踪与 Prompt 注册表关联<br>**AgentOps**: 包含会话回放与费率追踪的飞行记录仪监控 | **极极高**：通过 OTel 及 AgentOps 遥测，实现全生命周期会话回放、API 耗时/资费追踪以及死循环警告检测。 |
| **防退化与安全防御** | 无静态安全检查，完全信任 LLM 生成 | **OpenHands**: 拦截式 Security Analyzer 安全审查 | **高**：在命令执行器层增加拦截式安全检测插件，识别并拦截破坏性命令。 |
| **插件化与治理** | 硬编码在 scripts 目录，依赖特定接口 | **Semantic Kernel**: 标准化插件目录与依赖注入<br>**Pydantic AI**: 基于 Pydantic 强类型约束 of Tool 依赖注入 | **高**：将探针与动作插件化，引入 Pydantic AI 的 `RunContext` 类型安全依赖注入，保障多线程与单元测试的运行隔离。 |
