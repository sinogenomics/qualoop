...
### 1.3.4 目标导向主动调试与系统状态校验原则 (Goal-Oriented Active Debugging & Run-State Verification)

为了确保调试和改进意见的高置信度，Qualoop 发现问题与提出建议的机制必须严格贯彻**以最终目标为指针**、**校验系统真实运行态**的原则，杜绝测试空绿与 Mock 假绿：
- **目标指针化**：一切探测与改进项的设计必须能够追溯到 `GOALS.md` / `DEVELOPMENT_GOALS.md` 中的 North Star 目标媒介与功能边界，拒绝为了“表面绿色”而淡化或绕过业务目标。
- **运行态真实校验**：Tester 在发现问题时，不能仅依赖于静态的语法/格式扫描或简单的 HTTP 响应码（如 200 OK），必须像人类调试者一样，主动拉起/检查服务，并对系统运行状态、数据流转进行实质性探测。
- **产物完整性审计**：对于系统生成的任何目标媒介或交付文件（例如 PDF 文档、音频、视频、图表、JSON 结构体等），必须开展文件头魔术字节校验（如 `%PDF`, `ftyp`, `ID3`, `\x89PNG` 等）及实质性解析审计，严防空文件、损坏占位符绕过测试。
...
```
            ...
            ▼               ▼               ▼
     ┌─────────────┐  ┌───────────┐   ┌──────────────┐
     │  Architect  │  │Issue Store│◄──│  Scheduler   │
     │  (Planner)  │  │ (台账)    │   │ (唯一分配写)  │
     └──────┬──────┘  └─────▲─────┘   └──────┬───────┘
            │               │ score          │
            ├───────────────┤                ▼
            ▼               │         ┌──────────────┐
       ┌─────────┐──────────┘         │  Executors   │
       │ Tester  │  value_score*      │ fixer|improver│
       └────┬────┘  唯一写分者        │   |verifier  │
            ▼                         └──────────────┘
       ┌─────────┐
       │ Scorer  │
...
### 2.6 Architect / Planner（架构设计 / 规划者）

**职责**：面向项目北极星目标（North Star）或原始需求文档（如 `GOALS.md`），进行全局架构设计与里程碑拆解。

- 在项目初始化或新目标启动阶段被调用，分析全局目标并产出**总体方案/蓝图设计文档**（默认生成为 `docs/ARCHITECTURE_SCHEME.md`）。
- 将该蓝图拆解为一系列高层次的**里程碑任务（Milestone Issues）**，并以 `type: architecture` 写入 Issue Store。
- 随后由 Scheduler 进行指派，并由 Executors 执行具体设计、实现或脚手架搭建。
- **验证与门禁**：Architect 生成的规划方案和里程碑也必须经过 Scorer 的打分门槛，确保其方向不偏离 North Star，规避零散无序的自底向上式微调（improving）。
